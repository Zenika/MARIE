...
func (c MqttConnection) DoAll(thingType string, action string, params map[string]interface{}) {
	c.mqtt.Publish("type/"+thingType+"/action/"+action, []byte(""), 0, false)
}

// DoLocation do an action on all things with a specific location
func (c MqttConnection) DoLocation(thingType string, location string, action string, params map[string]interface{}) {
...
func (c MqttConnection) DoMacAddress(macaddress string, action string, params map[string]interface{}) {
